...
class State extends _1.Vertex {
     ...
     * @hidden
     */
    doEnterHead(transaction, deepHistory, trigger, next) {
        if (next) {
    ...
    doEnterTail(transaction, deepHistory, trigger) {
        this.regions.forEach(region => { region.doEnterHead(transaction); region.doEnterTail(transaction, deepHistory, trigger); });
        this.completion(transaction, deepHistory);
    }
    /**
