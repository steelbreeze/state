...
var Runtime = (function (_super) {
    ...
    Runtime.prototype.visitElement = function (element, deepHistoryAbove) {
        this.getActions(element).leave = delegate_1.create(this.getActions(element).leave, function (instance) { return logger.log(instance + " leave " + element); });
        this.getActions(element).beginEnter = delegate_1.create(this.getActions(element).beginEnter, function (instance) { return logger.log(instance + " enter " + element); });
    };
    Runtime.prototype.visitRegion = function (region, deepHistoryAbove) {
        ...
        if (deepHistoryAbove || !regionInitial || PseudoStateKind.isHistory(regionInitial.kind)) {
            ...
            this.getActions(region).endEnter = delegate_1.create(this.getActions(region).endEnter, this.getActions(regionInitial).beginEnter, this.getActions(regionInitial).endEnter);
        }
    };
