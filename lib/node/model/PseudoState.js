...
var PseudoState = /** @class */ (function () {
    ...
    PseudoState.prototype.when = function (guard) {
        return new Transition_1.Transition(this).when(guard);
    };
    /**
    ...
    PseudoState.prototype.to = function (target) {
        return new Transition_1.Transition(this, target, Transition_1.external);
    };
    /**
    ...
    PseudoState.prototype["else"] = function (target) {
        ...
        return this.elseTransition = new Transition_1.Transition(this, target, Transition_1.external).when(function () { return false; });
    };
    /** Find a transition from the pseudo state for a given trigger event */
    PseudoState.prototype.getTransition = function (trigger) {
        var _this = this;
        var result = (this.kind === PseudoStateKind_1.PseudoStateKind.Choice ? getTransition.Choice : getTransition)(this, trigger) || this.elseTransition;
...
function getTransition(pseudoState, trigger) {
    var result;
    ...
    for (var i = pseudoState.outgoing.length; i--;) {
        if (pseudoState.outgoing[i].evaluate(trigger)) {
            util_1.assert.ok(!result, function () { return "Multiple transitions found at " + pseudoState + " for " + trigger; });
...
getTransition.Choice = function (pseudoState, trigger) {
    var transitions = [];
