...
class Region {
    ...
    doEnterTail(transaction, deepHistory, trigger) {
        ...
        const starting = current && history(this, deepHistory, _1.PseudoStateKind.History) ? current : this.initial;
        if (starting) {
            starting.doEnter(transaction, history(this, deepHistory, _1.PseudoStateKind.DeepHistory), trigger);
        }
        else {
 ...
 * @hidden
 */
function history(region, deepHistory, kind) {
