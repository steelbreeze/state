...
class Region {
    ...
    doEnterTail(transaction, deepHistory, trigger) {
        ...
        const starting = deepHistory || this.is(_1.PseudoStateKind.History) && current ? current : this.initial;
        if (starting) {
            starting.doEnter(transaction, deepHistory || this.is(_1.PseudoStateKind.DeepHistory), trigger);
        }
        else {
