...
var Instance = /** @class */ (function () {
    ...
    Instance.prototype.transaction = function (operation) {
        try {
            ...
            var keys = Object.keys(this.dirtyState);
            for (var i = 0, l = keys.length; i < l; ++i) { // update the active state configuration
    ...
    Instance.prototype.evaluateDeferred = function () {
        ...
        this.deferredEventPool.forEach(function (trigger, i) {
            if (trigger && _this.root.getDeferrableTriggers(_this).indexOf(trigger.constructor) === -1) {
                delete _this.deferredEventPool[i];
                _1.log.write(function () { return _this + " evaluate deferred " + trigger; }, _1.log.Evaluate);
                if (_this.root.evaluate(_this, false, trigger)) {
                    _this.evaluateDeferred();
                    return;
                }
            }
    ...
    Instance.prototype.setVertex = function (vertex) {
        if (vertex.parent) {
            var regionName = vertex.parent.qualifiedName;
    ...
    Instance.prototype.setState = function (state) {
        if (state.parent) {
            var regionName = state.parent.qualifiedName;
