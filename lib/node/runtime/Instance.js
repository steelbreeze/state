...
var Instance = /** @class */ (function () {
    ...
    Instance.prototype.processDeferredEvents = function () {
        var _this = this;
        // build the list of deferred event types based on the active state configuration
        var deferrable = this.activeStateConfigurationDeferrableTriggers(this.root);
        var _loop_1 = function (i) {
            ...
            if (trigger && deferrable.indexOf(trigger.constructor) === -1) {
                delete this_1.deferredEventPool[i]; // NOTE: the transaction clean-up repacks the event pool
    ...
    Instance.prototype.activeStateConfigurationDeferrableTriggers = function (state) {
        var result = state.deferrableTrigger.slice();
