...
var Instance = /** @class */ (function () {
    ...
    Instance.prototype.evaluate = function (trigger) {
        ...
        var result = this.transaction(function () { return runtime_1.evaluate(_this.root, _this, false, trigger); });
        if (result) {
            // TODO: make the event pool and deferred items part of the transactional state
            var deferred_1 = this.eventPool.slice();
            this.eventPool = [];
            var _loop_1 = function (i) {
                util_1.log.info(function () { return _this + " evaluate " + deferred_1[i]; }, util_1.log.Evaluate);
                this_1.transaction(function () { return runtime_1.evaluate(_this.root, _this, false, deferred_1[i]); });
            };
            var this_1 = this;
            for (var i = 0; i < deferred_1.length; i++) {
                _loop_1(i);
            }
        }
        return result;
