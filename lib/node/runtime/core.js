...
function evaluate(instance, trigger) {
    ...
    instance.beginTran();
    var result = evaluateState(instance.root, instance, false, trigger);
    instance.commitTran();
    return result;
}
 ...
 */
function evaluateState(state, instance, deepHistory, trigger) {
    var result = false;
    // first, delegate to child states for evaluation
    for (var i = state.children.length; i--;) {
        if (evaluateState(instance.getState(state.children[i]), instance, deepHistory, trigger)) {
            result = true;
    ...
    if (result) {
        ...
        var transition = getVertexTransition(state, trigger);
        if (transition) {
