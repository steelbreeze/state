...
function getTransition(vertex, trigger) {
    ...
    for (var i = vertex.outgoing.length; i--;) {
        if (vertex.outgoing[i].guard(trigger)) {
            util_1.assert.undefined(result, function () { return "Multiple transitions found at " + vertex + " for " + trigger; });
...
model.Region.prototype.enterTail = function (instance, deepHistory, trigger) {
    ...
    util_1.assert.defined(starting, function () { return instance + " no initial pseudo state found at " + _this; });
...
model.PseudoState.prototype.getTransition = function (trigger) {
    ...
    util_1.assert.defined(result, function () { return "Unable to find transition at " + _this + " for " + trigger; });
    return result;
};
