...
function evaluate(state, instance, deepHistory, trigger) {
    ...
    var result = delegate(state, instance, deepHistory, trigger) || testTraverse(state, instance, deepHistory, trigger);
...
function testTraverse(vertex, instance, deepHistory, trigger) {
...
function getTransition(vertex, trigger) {
    ...
    for (var i = vertex.outgoing.length; i--;) {
        if (vertex.outgoing[i].guard(trigger)) {
            util_1.log.assert(!result, function () { return "Multiple transitions found at " + vertex + " for " + trigger; });
...
model.Region.prototype.enterTail = function (instance, deepHistory, trigger) {
    ...
    util_1.log.assert(starting, function () { return instance + " no initial pseudo state found at " + _this; });
...
model.PseudoState.prototype.getTransition = function (trigger) {
    ...
    util_1.log.assert(result, function () { return "Unable to find transition at " + _this + " for " + trigger; });
    return result;
};
