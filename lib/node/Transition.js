...
class Transition {
    ...
    constructor(source) {
        ...
        this.target = source;
        this.strategy = internalTransition(this.target);
    ...
    to(target, kind = _1.TransitionKind.External) {
        ...
        if (kind === _1.TransitionKind.External) {
            ...
            this.strategy = localTransition(this.target);
        }
        return this;
    }
    /**
 ...
 * @hidden
 */
function internalTransition(target) {
    return (transaction, deepHistory, trigger, actions) => {
        _1.log.write(() => `${transaction.instance} traverse internal transition at ${target}`, _1.log.Transition);
        actions.forEach(action => action(trigger, transaction.instance));
        if (target instanceof _1.State) {
 ...
 * @hidden
 */
function localTransition(target) {
    return (transaction, deepHistory, trigger, actions) => {
        _1.log.write(() => `${transaction.instance} traverse local transition to ${target}`, _1.log.Transition);
